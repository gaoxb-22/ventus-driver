...
- Downstream (devices): supports multiple (simulation) backends:
  ...
  - Verilator-based GDB debug backend built on `libVentusGDB.so`, path: driver/gdb_device
...
- `VENTUS_BACKEND` selects the device backend. Allowed values: spike/isa, rtl/rtlsim/gpgpu, gdb/debug, cycle/cyclesim/systemc/simulator.
...
- 向底层连接多种（仿真）设备，目前支持：
  ...
  - 基于 `libVentusGDB.so` 的 Verilator GDB 调试后端，对应 driver/gdb_device
...
### 环境变量
- VENTUS_BACKEND 选择底层设备，可选值：spike/isa，rtl/rtlsim/gpgpu，gdb/debug，cycle/cyclesim/systemc/simulator。
